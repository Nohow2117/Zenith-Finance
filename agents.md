# Agents Rules — ArtDeFinance

> **Repository:** [github.com/Nohow2117/ArtDeFinance](https://github.com/Nohow2117/ArtDeFinance)
>
...
| Hosting      | VPS Linux (Node.js standalone)  | Vercel, AWS Lambda, Docker (salvo approvazione) |
...
| Parametro        | Valore                                      |
| ---------------- | ------------------------------------------- |
| Host             | `185.165.169.119`                           |
| User SSH         | `artdefinance`                              |
| Chiave SSH       | `C:\Users\fede1\.ssh\id_ed25519.pub`       |
| Dominio          | `artdefinance.com`                          |
| Comando SSH      | `ssh artdefinance@185.165.169.119`          |
| Runtime          | Node.js (Next.js standalone output)         |
| Reverse Proxy    | Nginx o Caddy (HTTPS con Let's Encrypt)     |
| Process Manager  | PM2 o systemd                               |

### Procedura di Deploy

Deploy ufficiale del progetto:

```powershell
powershell -ExecutionPolicy Bypass -File .\deploy.ps1
```

Il deploy DEVE essere release-based con:
- artifact unico compresso;
- directory `releases/` versionate;
- directory `shared/` per `.env` e `local.db`;
- symlink `~/app` verso la release corrente;
- `pm2 reload` o equivalente solo dopo health check locale riuscito;
- possibilità di rollback rapido alla release precedente.

Struttura remota attesa:
...
- Nginx/Caddy configurato come reverse proxy su porta 3000
- Certificato SSL via Let's Encrypt per `artdefinance.com`
- File `.env` presente in `~/app/.env` con tutte le variabili di produzione
  Nota aggiornata: `~/app/.env` deve essere un symlink verso `shared/.env`, non un file copiato dentro la release
